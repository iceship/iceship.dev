...
> **Note**: Build a documented / type-safe API with hono, drizzle, zod, OpenAPI
> and scalar
...
> **Tip**: 여기서 template는 nodejs로 사용 (추후 cloudflare workers로 변경)
...
> **Tip**: framework는 none 선택 (backend라서 따로 framework는 선택 안 함)
...
> **Tip**: `src/index.ts`에 `// eslint-disable-next-line no-console`를 넣어서
> console.log 에러가 나는 걸 우선 막아준다.
...
[Zod OpenAPI Hono](https://github.com/honojs/middleware/tree/main/packages/zod-openapi#zod-openapi-hono)
사용해서 OpenAPI 구현
...
[CJ stoker](https://github.com/w3cj/stoker) Hono library install
...
간단한 log 기록을 위해서 hono에서 제공하는 logger를 사용해도 되지만, 좀 더
자세한 정보를 위해 pino를 사용

#### pino 설치

[pino](https://github.com/pinojs/pino/blob/HEAD/docs/web.md#hono) hono에서 pino
사용 방법
...
> **Error**: Property 'logger' does not exist on type
> 'Readonly\<ContextVariableMap & object\>'.ts(2339)

`src/lib/types.ts` 만들고:

> **Info**: 프로젝트 전반의 타입 안정성과 개발 편의성을 높이기 위해, Hono의 기본
> 타입들을 우리 앱에 맞게 래핑(wrapping)한 유틸리티 타입을 정의한다.
...
`src/lib/create-app.ts` 만들고 OpenAPIHono를 사용할 때 제네릭으로 `AppBindings`
타입 지정
...
> **Tip**: `package.json`에 `"test": "cross-env NODE_ENV=test vitest"` 추가
